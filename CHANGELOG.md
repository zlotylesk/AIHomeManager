...
Wydanie techniczne zamykające epik **HMAI-422** — trzynaście zadań (HMAI-423…435) z przeglądu po 1.34.0. Bez nowego modułu domenowego i bez nowej funkcjonalności użytkownika.

Motyw przewodni: **wszystkie bramki jakości sprawdzają kod, żadna nie sprawdzała sposobu jego uruchomienia.** Konfiguracja produkcyjna po prostu nie istniała — jeden plik Compose, deweloperski, a działający stack raportował `Environment: dev, Debug: true`. Zmierzone na żywym środowisku: **3,8 s na odpowiedź 401**, 5,5 s na kontrolę zdrowia, **16,7 s na zrzut kontraktu OpenAPI**. CI tego nie zobaczy z założenia: testy jadą w środowisku `test` na maszynie GitHuba, nigdy wewnątrz obrazu, w którym aplikacja faktycznie działa — ta sama martwa strefa, dla której powstało `make doctor`.

Ten sam przegląd potwierdził na żywo, że **cała infrastruktura nasłuchiwała na wszystkich interfejsach**: baza przyjmująca konto root z hasłem leżącym w śledzonym pliku repozytorium, Redis z pustym `requirepass`, broker akceptujący `guest:guest` spoza pętli zwrotnej wraz z panelem zarządzania odpowiadającym kodem 200 z poziomu hosta. Na maszynie deweloperskiej bez znaczenia; na maszynie z publicznym adresem to natychmiastowe przejęcie danych bez zgadywania hasła.

Trzeci wątek to powtórka wzorca z 1.31.0 po raz trzeci: mechanizmy wykrywania powstawały i działały, ale **nikt ich nie odpytywał**. Kopie zapasowe leżały wyłącznie na tym samym dysku co baza, nieszyfrowane, a odtworzenie z nich nigdy nie zostało wykonane — sprawdzano rozmiar i wiek pliku, czyli wszystko poza jedyną rzeczą, która ostatecznie się liczy. Detekcja jest budowana za każdym razem; **ostatni metr — to, że ktoś się dowiaduje — nie był budowany nigdy.**

- **+213 testów PHP** (2690 wobec 2477 w 1.34.0), Playwright bez zmian (169), Vitest bez zmian (249). PHPStan level 8 clean, deptrac **0 violations / 0 skip_violations**, zero nowych wpisów w `phpstan-baseline`. Liczba asercji Newman podawana w dokumentacji była błędna w obie strony — kolekcja nie była ruszana od 1.34.0, a bramka raportuje niezmiennie **161**.

### Added

- **Obraz produkcyjny i środowisko `prod`** (HMAI-423). `docker/php/Dockerfile` jest wieloetapowy nad kontekstem z katalogu głównego: wspólny `base` (żeby oba obrazy nigdy nie różniły się tym, *które rozszerzenia istnieją*), pusty `dev`, `assets` z produkcyjnym buildem Encore'a i `prod` z kodem skopiowanym do środka, `composer install --no-dev` i cache rozgrzanym na etapie budowania. nginx ma własny obraz z `public/` w środku, bo `try_files` rozstrzyga pliki statyczne na własnym systemie plików nginx-a — i bierze ten katalog **z obrazu aplikacji, nie z kontekstu budowania**: `public/build/` i `public/bundles/` są generowane i ignorowane przez git, więc kopiowanie ich z kontekstu dawało działający obraz na maszynie, która akurat je miała, i w połowie pusty na czystym klonie. Stąd `make prod-build` to dwa uporządkowane kroki. Wynik: kontrola zdrowia **5,19 s → 13 ms**, odpowiedź 401 **3,8 s → 7 ms**.

  **Korekta wobec opisu zadania: OPcache nigdy nie był wyłączony.** `php:8.5-fpm-alpine` dostarcza go wkompilowanego z `opcache.enable => On`; żadne `docker-php-ext-enable opcache` nie było potrzebne ani nie brakowało. Realnymi brakami były `validate_timestamps => On` (jedno `stat()` na dołączony plik na żądanie) oraz `max_accelerated_files => 10000` przy 50 847 plikach PHP. Dominującym kosztem tych 3,8 s było `APP_ENV=dev` z debugiem, a nie kompilacja bajtkodu.

- **HTTPS dla ruchu przeglądarkowego** (HMAI-424). TLS terminuje w nginx, wyłącznie na produkcji — deweloperka zostaje na `:8080` po HTTP, bo przeglądarka i tak traktuje localhost jako kontekst bezpieczny, a wszystkie zestawy E2E gadają właśnie z nim. Obie konfiguracje serwisu `include`ują te same `snippets/`, więc reguły serwowania nie mogą się rozjechać. To nie jest wyłącznie kwestia bezpieczeństwa: Google, Spotify i Trakt nie przyjmują adresu zwrotnego po `http://` poza pętlą zwrotną, więc bez HTTPS cztery moduły w ogóle nie ruszają na produkcji.

  **Publiczna nazwa hosta występuje w repozytorium dokładnie raz** — jako argument `DOMAIN=` do `make prod-cert-init`. Certyfikaty wystawiane są pod stałą nazwą linii `aihm`, więc nginx nazywa linię, a nie domenę, i żadna konfiguracja nie jest szablonowana. Skrypt wejściowy podlinkowuje tę linię przy starcie albo wypisuje samopodpisany zastępnik: nginx odmawia startu bez certyfikatu, a jedyny sposób na jego uzyskanie to wyzwanie obsłużone przez **działający** nginx — zastępnik przerywa ten zakleszczenie na świeżym hoście.

- **Kopie zapasowe poza hostem, szyfrowane, z wykonanym odtworzeniem** (HMAI-426). Przechowywany artefakt **jest** zaszyfrowany — to format, a nie krok po drodze: `homemanager-YYYY-MM-DD.sql.gz.enc`, gzip w środku (zaszyfrowane bajty się nie kompresują), libsodium `secretstream` na zewnątrz. `secretstream`, a nie `secretbox` używany przez `TokenCipher`, bo szyfr nad całym łańcuchem trzymałby bazę w pamięci dwa razy; jawny zrzut istnieje wyłącznie jako plik tymczasowy z kropką na początku i prawami 0600, którego nie łapie żaden glob i który jest usuwany w `finally`.

  **Odszyfrowanie nalega na znacznik ostatniego bloku.** Wysyłka ucięta dokładnie na granicy bloku odszyfrowuje się inaczej czysto do niepełnego zrzutu, a niepełny zrzut odtwarza bazę po cichu pozbawioną wszystkiego po miejscu cięcia. Miejsce docelowe jest portem z fabryką (`none` | `directory` | `rclone`), bo alias rozstrzyga się na etapie kompilacji kontenera i nie potrafi czytać zmiennej środowiskowej, a nieznana wartość jest odrzucana przy starcie zamiast degradować do `none` — co dałoby instancję wyglądającą na skonfigurowaną i niekopiującą niczego. Backend `directory` **porównuje identyfikatory urządzeń, nie ścieżki**: „zdalny" katalog na dysku samej bazy nie chroni przed niczym. `make restore-drill` odtwarza najnowszy artefakt do osobnego schematu i porównuje liczby wierszy, bo kontrole świeżości i rozmiaru potrafią przejść na pliku, którego już nie da się odtworzyć.

- **Powiadamianie o awarii** (HMAI-432). `src/Monitoring/` — sondy na timerze, e-mail na wyjściu. **Nie przechodzi przez moduł Notifications i to jest cały sens tego rozwiązania**: tamten silnik czyta wiersz preferencji i zapisuje wiersz powiadomienia przed wysłaniem, co jest poprawne dla powiadomień użytkownika i **niezdolne** do zaanonsowania niedziałającej bazy, bo anonsowanie wymagałoby bazy. Alerting sięga więc wprost do Mailera, trzyma stan deduplikacji w **pliku JSON na dysku lokalnym** i składa treść w PHP zamiast w Twigu. Nic na tej ścieżce nie dotyka MySQL-a, Redisa ani RabbitMQ; `AlertDeliveryIndependenceTest` psuje wszystko naraz i nadal żąda maila.

  **Przegląd anonsuje przejścia, nie stan.** Sondy raportują wszystko, co nie działa, przy każdym przebiegu; tygodniowa awaria ma kosztować jeden e-mail. Severity, które **rośnie**, jest anonsowane ponownie (82% i 96% zajętości dysku to dwie różne sytuacje), spadające — nie. Alert, którego nie przyjął żaden kanał, nie zostaje odnotowany, więc awaria poczty opóźnia go, a nie połyka. **Sonda, która rzuci wyjątkiem, nie rozwiązuje własnych alertów** — inaczej pierwszym aktem zepsutej sondy byłoby odwołanie wszystkiego, co dotąd obserwowała.

### Security

- **Zamknięcie portów infrastruktury i poświadczenia brokera** (HMAI-425). Produkcja publikuje port 80 i 443 nginx-a i nic poza tym; deweloperka publikuje wszystko i ma tak zostać — klienci na hoście i serwery MCP gadają z 3306/6379/5672/15672/9200, a maszyna deweloperska nie jest tym, przed czym to chroni. Nakładka wpisuje `ports: !override []`, bo **Compose dokleja nieotagowane sekwencje**: nakładka wyłącznie wymieniająca dowiązania do pętli zwrotnej opublikowałaby je **oraz** mapowania `0.0.0.0` z pliku bazowego.

  Trzy rzeczy zmierzone tutaj, z których każda kosztowałaby ponowne odkrycie. **`redis-cli ping` przeciwko serwerowi z hasłem wypisuje `NOAUTH` i kończy się kodem 0**, więc standardowa kontrola zdrowia raportowała kontener jako sprawny, podczas gdy odrzucał on każdego klienta — sonda musi czytać odpowiedź (`| grep -q PONG`), a nie kod wyjścia. **`loopback_users.guest = false` jest wypieczone w obrazie brokera** w `conf.d` i nie wywodzi się z `RABBITMQ_DEFAULT_USER`, więc przemianowanie konta domyślnego nie ogranicza `guest`. **`RABBITMQ_DEFAULT_USER`/`PASS` działają wyłącznie przy inicjalizacji pustej bazy** — na istniejącym wolumenie są po cichu ignorowane, a konto trzeba dodać przez `rabbitmqctl` i `guest` usunąć ręcznie.

- **Sekrety produkcyjne poza śledzonymi plikami** (HMAI-427). Poświadczenia infrastruktury żyją w `.env` z katalogu głównego, a nie w `app/.env.local`, bo Compose czyta je **budując stack, zanim istnieje kontener zdolny przeczytać plik środowiskowy aplikacji**. Śledzone wartości są deweloperskie; `make prod-*` nakłada na nie ignorowany przez git `.env.local` (`--env-file .env --env-file .env.local`, wygrywa drugi), więc wdrożenie nie wymaga edycji żadnego śledzonego pliku.

  **Warstwowanie zawodzi po cichu w jedną stronę i właśnie to pokrywa `make doctor`.** Zmienna *pominięta* w `.env.local` nie zatrzymuje wdrożenia — spada do wartości deweloperskiej i instancja wstaje zdrowa, na haśle opublikowanym w publicznym repozytorium. Nic w działającym stacku nie wygląda źle. Kontrola nazywa każdą zmienną wciąż stojącą na wartości z `.env`, porównując z tym, co `.env` **aktualnie** zawiera, a nie z kopią tych łańcuchów. Zmierzone przy okazji: **zduplikowany klucz w pliku dotenv rozstrzyga się do ostatniej definicji** — `scripts/doctor.sh` czytał pierwszą w pięciu miejscach i dawał fałszywe OK, ilekroć operator dopisał poprawkę zamiast edytować w miejscu. Sygnałem „czy ten host wdraża" jest obecność projektu Compose `aihm-prod`, a nie sama obecność `.env.local` — stacja robocza trzyma go legalnie dla tokenu MCP, a kontrola czerwona na każdej maszynie deweloperskiej przestaje być czytana.

  **Przegląd historii repozytorium pod kątem sekretów: czysto, nic do unieważnienia.**

- **Content Security Policy jako druga warstwa broniąca klucza API** (HMAI-433). Polityka istnieje, bo **każda strona niesie klucz API w znaczniku `meta`, a interfejs buduje znaczniki przez `innerHTML` w około 130 miejscach** — jedno pominięte `escHtml` jest zwykle błędem układu, ale przy tej kombinacji jest pełnym przejęciem API. Polityka jest wyłącznie nagłówkiem, nigdy znacznikiem `meta`: `meta` w ogóle nie potrafi nieść `frame-ancestors`, a dwie polityki łączyłyby się przez przecięcie. **Mapa nginx-a jest kluczowana po `$request_uri`, nie po `$uri`**, bo `try_files` przepisuje `$uri` na `/index.php` zanim powstanie odpowiedź budowana przez PHP — mapa po `$uri` nigdy nie zobaczyłaby `/api/doc` i po cichu podawałaby politykę ścisłą dla każdej strony dynamicznej.

- **Rotacja klucza API i ograniczenie prób logowania** (HMAI-434). **Rotacja przyjmuje dwa klucze, nigdy jednego.** `API_KEY_PREVIOUS` jest opcjonalną drugą wartością porównywaną również przez `hash_equals`, przy czym **oba porównania wykonują się bezwarunkowo**, bez skrótu logicznego, więc to, który z dwóch pasował, nie jest obserwowalne z czasu odpowiedzi. Pusta wartość oznacza brak trwającej rotacji i nigdy nie degeneruje się do dopasowania pustego łańcucha, bo żądanie bez nagłówka jest odrzucane własną ścieżką, zanim którekolwiek porównanie ruszy. `login_throttling` bramkuje nieudane logowania do panelu: 5 prób na parę użytkownik+IP i 25 na samo IP, oba na 15 minut. Żądanie zablokowane i błędne hasło są nierozróżnialne z sieci.

### Fixed

- **Sonda dysku mierzyła nie ten system plików** (HMAI-431). Dysk jest teraz **dwoma komponentami i mierzy ścieżki, a nie kontener, w którym działa**: `disk_database` czyta system plików trzymający `DATABASE_DATA_DIR`, `disk_backups` ten trzymający `BACKUP_DIR` — to, który z dwóch się zapełnił, rozstrzyga, czy przycięcie zrzutów w ogóle pomoże. Te same progi, inne znaczenie stanu krytycznego: wyłącznie `disk_database` zamienia go w `down`, bo MySQL bez zapasu miejsca umiera na flushu i binlogu. Główny system plików kontenera PHP nie odpowiada za żaden z nich, więc `php` i `scheduler_worker` montują `mysql_data` **tylko do odczytu** pod `DATABASE_DATA_DIR`. **Nieudany pomiar to `degraded`, nigdy `down`** — niewiedza o tym, ile miejsca zostało, to nie to samo co wiedza, że nie ma go wcale.

- **Okno retencji kopii poza hostem było w produkcji o dzień krótsze niż w testach** (HMAI-422, przegląd epiku). `RemoteRetention` porównywała datę kopii — parsowaną przez `BackupFilename` z `!`, więc zawsze północ — z `$today` niosącym zegar ścienny, bo `DatabaseBackupService::pushOffsite()` przekazuje `new DateTimeImmutable()`, a zrzut leci o 03:00. Kopia sprzed dokładnie `keepDays` dni lądowała po tej stronie odcięcia, którą wskazała godzina: **zachowana o 00:00, skasowana o 03:00**. Nocny przebieg i ręczne `make backup-now` przycinały więc różne zestawy plików w tym samym katalogu. Żaden istniejący test nie mógł tego zobaczyć — obie destynacje sięgają tej klasy przez `prune()`, ale wyłącznie z samą datą. `$today` jest teraz normalizowane do początku doby: retencja liczy się w pełnych dniach, a godzina, o której jest liczona, przestaje być częścią odpowiedzi.

- **Trzy defekty widoczne wyłącznie przy instalacji `--no-dev`** (HMAI-423). Ani deweloperka, ani testy nie mogą ich zobaczyć, bo oba środowiska instalują `require-dev`. `routes.yaml` skanował całe `../src/`, a loader atrybutów tras **refleksuje** każdą klasę, ładując `AppFixtures`, którego klasa nadrzędna jedzie w `doctrine/doctrine-fixtures-bundle`; `services.yaml` rejestrował `App\` bez `exclude`, dając ten sam błąd krytyczny drugą drogą. Stąd reguła: **klasa pod `src/` dziedzicząca po klasie z `require-dev` musi być wykluczona z obu skanów** — trasowania i usług — i ponownie zarejestrowana pod `when@dev` / `when@test`.

### Changed

- **Polityki restartu i limity pamięci** (HMAI-428). Każda usługa niesie `restart: unless-stopped` oraz `mem_limit`, więc restart hosta podnosi stack bez ręcznego kroku i żadna usługa nie zagłodzi pozostałych. Polityka restartu stoi w pliku bazowym jako skalar, więc nakładka produkcyjna dziedziczy ją nietkniętą. `mem_limit` jest natomiast wyłącznie produkcyjny, a nie wspólny: `php` i `scheduler_worker` są też miejscem, gdzie `docker exec` uruchamia cięższe narzędzia lokalne (samo `make phpstan` prosi o `--memory-limit=1G`), a sufit dobrany pod działającą aplikację zagłodziłby je na maszynie deweloperskiej.

- **Rotacja logów kontenerów i limity wolumenów** (HMAI-429). Każda usługa ma w sterowniku `logging:` ustawione `max-size`/`max-file`, bo domyślnie jest to nieograniczone, a log leży na tym samym dysku co baza. `app/var/log` omija ten sterownik w całości — to plik montowany, nie stdout/stderr — więc uchwyt `dev` w `monolog.yaml` jest typu `rotating_file` z `max_files: 7` zamiast zwykłego `stream`, który urósł nierotowany do **640 MB**. Produkcyjny kanał `deprecation` jest typu `null`: był jedynym uchwytem nigdy nieopakowanym w `fingers_crossed`, więc wypisywał każde ostrzeżenie o wycofaniu wprost na stderr, nie mając za sobą niczego, co dałoby się na produkcji zrobić.

- **Limit pamięci Redisa i polityka wypierania kluczy** (HMAI-430). Redis jedzie z `maxmemory 192mb` i `maxmemory-policy allkeys-lru`, a nie `volatile-lru`. Każdy klucz tej instancji ma dziś TTL, więc obie polityki wypierają identycznie; `allkeys-lru` jest wybrane mimo to, bo **zawsze ma co wypierać**, podczas gdy `volatile-lru` degraduje się do odrzucających zapisy błędów `noeviction` w chwili, w której przestrzeń kluczy kiedykolwiek zapełni się kluczami bez wygasania. Stan samego harmonogramu nie leży w Redisie w ogóle — `Schedule::stateful()` jedzie na puli `cache.app` opartej o system plików.

- **Klucz szyfrowania kopii dostępny dla zestawu testów** (HMAI-422, przegląd epiku). Dotenv Symfony celowo pomija `.env.local` w środowisku `test`, więc `BACKUP_ENCRYPTION_KEY` istniał wyłącznie w pliku nieśledzonym: `make test` na świeżym klonie padał na czterech testach, podczas gdy CI pozostawało zielone, wstrzykując klucz jako zmienną joba. Klucz testowy leży teraz w śledzonym `app/.env.test`, obok czterech kluczy tokenów, które były tam od dawna.

### Documentation

- **Synchronizacja dokumentacji po epiku** (HMAI-435). README, CLAUDE.md, `docs/` i strony uruchomieniowe w Confluence doprowadzone do stanu po zmianach. Zmierzone, a nie przeczytane: **siedem commitów epiku zaktualizowało CLAUDE.md o 1 924 słowa i ruszyło `docs/`, ale nie tknęło `README.md` ani razu** — reguła „aktualizuj CLAUDE.md w commicie zadania" po cichu zostawia README jako najbardziej nieaktualną powierzchnię, więc to ją należy sprawdzać pierwszą, a nie ostatnią. Najgorsze znalezisko leżało poza zakresem zadania: runbook odtwarzania po awarii — strona czytana w trakcie faktycznej awarii — twierdził, że cel Make z założenia nigdzie nie wypycha kopii, i podawał format jako `.sql.gz`, czyli dokładnie odwrotnie, niż robi kod.

## [1.34.0] — 2026-08-05

Wydanie jakościowe zamykające epik **HMAI-409** — dwanaście zadań (HMAI-410…421) z przeglądu aplikacji po 1.33.0. Bez nowego modułu domenowego i bez nowej funkcjonalności użytkownika.

Motywem przewodnim jest to samo, co w 1.31.0: **awarie, które udają sukces**. Ten przegląd znalazł ten sam wzorzec w nowych miejscach. Kopia zapasowa, która przestała powstawać, przechodziła kontrolę zdrowia, bo `make doctor` sprawdzał rozmiar najnowszego pliku, a nie jego wiek — i przez sześć dni (29.07–03.08) raportował `OK` przy harmonogramie, który nie odpalił ani razu. Kolejka błędów, która ma czekać na człowieka, znikała przy odtworzeniu kontenera brokera, choć zwykły restart jej nie ruszał. Ta sama passa aktywności miała dwie różne wartości na dwóch ekranach tej samej aplikacji. Sonda `/api/health` odpowiadała sześcioma zielonymi komponentami przy obu martwych workerach, bo pytała brokera o dostępność, a broker jest w pełni zadowolony, gdy nikt nie konsumuje.

Osobno: dokumentacja i plik kontekstu urosły do rozmiarów, które utrudniały jedno i drugie — uruchomienie projektu i pracę nad nim. README miał 869 linii, a jego sekcja „Quick start" zaczynała się w linii 152. CLAUDE.md miał 45 353 słowa, z czego 65% stanowiła narracja historyczna.

- **+67 testów PHP** (2477 wobec 2410 w 1.33.0), **+8 Playwright** (169), **+31 Vitest** (249), Newman bez zmian (121 — kolekcja została przepisana pod nową kopertę odpowiedzi, nie rozszerzona). PHPStan level 8 clean, deptrac **0 violations / 0 skip_violations**, zero nowych wpisów w `phpstan-baseline`.

### Fixed

- **Kontrola świeżości kopii zapasowej** (HMAI-410). `make doctor` sprawdzał wyłącznie rozmiar najnowszego zrzutu, więc harmonogram, który przestał odpalać, zostawiał w katalogu całkowicie poprawny plik i przechodził kontrolę bez końca. Doszło sprawdzenie **wieku**, z progiem **48 godzin**, a nie 24: cron o 03:00 nie jest momentem, w którym backup faktycznie powstaje na stacji roboczej wyłączanej na noc — Scheduler odpala przegapione okno przy najbliższym starcie hosta, więc realne zrzuty lądują między 07:00 a 22:00, a próg dobowy krzyczałby wilk w zwyczajny dzień. Wiek czytany jest z **daty w nazwie pliku, nie z mtime**: kopiowanie albo synchronizacja katalogu stempluje każdy plik bieżącą chwilą, więc kontrola po mtime uznałaby archiwum sprzed miesięcy za świeże — dokładnie ta klasa uspokajająco błędnej odpowiedzi, którą to sprawdzenie ma usuwać. Trzecie sprawdzenie **ostrzega**, ile z przechowywanych kopii jest pustych; przy pierwszym uruchomieniu było to 1 z 27.
- **Trwałość kolejki błędów** (HMAI-411). Topologia nigdy nie była problemem — transport AMQP Symfony deklaruje `AMQP_DURABLE` i ustawia `delivery_mode: 2`, więc kolejki, wymiany i komunikaty są trwałe, i właśnie dlatego zwykły `docker compose restart` nigdy niczego nie tracił, a luka pozostawała niewidoczna. Trwałość zapisuje się do `/var/lib/rabbitmq`, a bez wolumenu żyło to w zapisywalnej warstwie kontenera: każde **odtworzenie** kontenera (`down`/`up`, podbicie obrazu, dowolna edycja pliku compose) kasowało DLQ. Doszedł nazwany wolumen **oraz ustalony `hostname`** — i obie rzeczy są konieczne. RabbitMQ wyprowadza nazwę węzła z hostname'a i trzyma bazę każdego węzła w `mnesia/rabbit@<hostname>`, więc przy domyślnym identyfikatorze kontenera każde odtworzenie startowałoby **pusty nowy węzeł**, podczas gdy dane poprzedniego leżałyby nietknięte obok w tym samym wolumenie: montowanie wyglądałoby poprawnie, a komunikaty i tak by zniknęły.
- **Jedno źródło prawdy dla passy aktywności** (HMAI-412). `/api/goals/streaks` i kokpit `/` czytały passę dwiema różnymi drogami i podawały różne liczby. Obie czytają teraz przez wspólny port `App\Shared\Activity\StreakReaderInterface`. Strategia: **bieżąca seria liczona na żywo, rekord wszech czasów dołączony z utrwalonego wiersza** — żadna z połówek nie wystarcza sama. Czytanie wyłącznie utrwalonej tabeli zostawia kokpit o noc w tyle (jej jedynym zapisującym jest nocne przeliczenie), a odświeżanie jej przy rejestracji aktywności wymagałoby szyny zdarzeń, której **pięć modułów źródłowych nie jest w stanie zasilić** — Artykuły, Muzyka, Filmy, Podcasty i YouTube nie emitują żadnych zdarzeń domenowych, więc mechanizm pokrywałby część aktywności i po cichu gubił resztę. Liczenie wyłącznie na żywo gubi z kolei każdy rekord starszy niż okno 365 dni. Regresja: `StreakConsistencyTest` czyta **obie** powierzchnie HTTP na jednym zestawie danych i sprawdza, że się zgadzają — nic w zestawie testów nigdy ich wcześniej nie porównało, i dlatego rozjazd przetrwał.
- **Sonda `/api/health` nie widziała martwego workera** (HMAI-418). Komponent `rabbitmq` otwiera gniazdo do brokera, a broker jest w pełni zadowolony, gdy nikt nie konsumuje — instancja z dwoma martwymi workerami raportowała sześć zielonych komponentów, podczas gdy każdy import, powiadomienie, aktualizacja indeksu i nocny backup po cichu stanęły. To ta sama martwa strefa, która pozwoliła harmonogramowi backupu milczeć przez sześć dni. **Pytanie RabbitMQ o liczbę konsumentów też by nie pomogło**: scheduler konsumuje `scheduler_default`, transport Symfony Scheduler, który nigdy nie dotyka brokera — więc jedyny worker, którego śmierć faktycznie kosztowała, jest stamtąd niewidoczny. Workery raportują więc same o sobie: heartbeat do Redisa z własnej pętli, jeden klucz na transport, a komponent jest `up` tylko wtedy, gdy **każdy** transport odezwał się w ciągu 5 minut — wszystkie, nie którykolwiek, bo w tej awarii worker asynchroniczny żył, a scheduler nie. Zgłasza `degraded` (HTTP 200), nigdy `down`: instancja nadal obsługuje każde żądanie, a naprawą jest restart workera, nie przekierowanie ruchu.
- **Waluta w module Budget** (HMAI-416). Agregacja ignorowała kod waluty, więc raport miesięczny mógł zsumować kwoty w różnych walutach i podać jedną liczbę. Budżet jest teraz prowadzony w **dokładnie jednej** walucie, a kwota w innej jest odrzucana z 422 — nie przeliczana i nie przemianowywana po cichu. Pełna wielowalutowość została **odrzucona świadomie, a nie odłożona**: to nie jest kolumna z walutą, tylko kursy wymiany, ich historia, data kursu dla transakcji sprzed miesiąca, waluta na kategorii *i* na jej limicie oraz odpowiedź na pytanie, w czym w ogóle jest denominowane saldo mieszanego miesiąca — a połowa tego mechanizmu to znowu kwota, która może być nieprawdziwa. Raport **niesie teraz walutę swoich liczb** (wcześniej odpowiadał gołym `20000`) i **odmawia policzenia miesiąca, który miesza waluty**, zamiast go zsumować: zapisy czynią ten stan nieosiągalnym przez API, ale „nie może się zdarzyć" to nie to samo co „jest sprawdzane", a uczciwej pojedynczej liczby dla takiego miesiąca nie ma.

### Performance

- **Indeksy pod zapytania zakresowe adapterów międzymodułowych** (HMAI-413). Goals, Insights, kokpit i raport tygodniowy czytają `articles`, `series_episodes`, `book_reading_sessions`, `videos`, `music_listening_sessions` i `tasks` po oknie czasowym przy każdej wizycie na `/goals`, `/insights` i stronie głównej oraz w nocnym przeliczeniu — bez indeksu jest to pełne skanowanie tabeli rosnące wraz z biblioteką, przy **całkowicie poprawnej odpowiedzi**. Nic się nie psuje, robi się tylko wolniej, i właśnie dlatego żaden test funkcjonalny nigdy by tego nie zauważył. Predykaty złożone są indeksowane **równością na początku** (`(is_read, read_at)`, `(watched, watched_at)`, `(date, pages_read)`), bo `watched = 1 AND watched_at BETWEEN …` potrafi użyć obu połówek tylko w tej kolejności; kolumna zamykająca jest zarazem całością tego, co te zapytania wybierają, więc każdy z tych odczytów jest obsługiwany **wyłącznie z indeksu** (`Using index`, bez sięgania po wiersze). `RangeQueryIndexTest` uruchamia **prawdziwe** adaptery i czyta licznik `Handler_read_rnd_next` samego MySQL-a zamiast powtarzać ich SQL, więc predykat przerobiony później na kształt, którego indeks nie obsłuży, oblewa test, choć nadal zwraca poprawne wiersze.

### Changed

- **Stronicowanie endpointów listujących** (HMAI-415). Jedna konwencja na całe API: zapytanie kolekcyjne niesie `PageRequest` (`page` od 1, `perPage` domyślnie 50 / maks. 100), a handler zwraca `Page` — `COUNT(*)` po przefiltrowanym zbiorze plus `LIMIT/OFFSET`. Odpowiedź to koperta `{data, pagination}`; wartość poza zakresem to **422, nigdy ciche przycięcie**. Odczyt rozgałęziający się po złączeniu (Serial → sezony → odcinki) stosuje okno do **rodzica** w podzapytaniu, nigdy do złączonych wierszy — inaczej strona przecina agregat na pół. Źródło, które potrafi oddać wyłącznie cały zbiór (buforowana kolekcja Discogs), okienkuje go przez `Page::slice()`: to ogranicza odpowiedź, czyli dokładnie to, czym ograniczone są klient mobilny i budżet cache'u PWA, bez udawania, że odczyt w górę stał się tańszy. Świadomie niestronicowane zostają odczyty ograniczone enumem, ograniczone jawnym oknem oraz złożone read modele, które nie są kolekcjami.
- **Jedna implementacja helperów frontendowych zamiast czterech kopii** (HMAI-414). `escHtml` istniał w czterech kopiach, zabezpieczając około trzydziestu zapisów `innerHTML` — w tym jeden nad tekstem zaimportowanym z Pocketa — a `showError`/`showInfo` w sześciu. Trzy panele na starym torze dostają teraz **te same obiekty funkcji** przez `assets/legacy-globals.js`, a nie własne warianty. Działa to, bo `base.html.twig` renderuje `encore_entry_script_tags('app')` **przed** `{% block javascripts %}`, a Encore emituje klasyczne, nieodroczone znaczniki skryptu — ta kolejność jest nośna, i dodanie `defer` albo `type=module` do wyjścia Encore'a zepsułoby wszystkie stare panele naraz. Doszła też bramka `node --check` nad trzema plikami starego toru: nie parsuje ich nic innego — nie są w buildzie Encore'a ani w globie Vitesta — więc błąd składni przeżywał każdą inną bramkę i pojawiał się dopiero jako martwy panel w przeglądarce. To nie jest hipotetyczne: zbiorcza edycja w trakcie tego zadania przerobiła trzy deklaracje funkcji na `function window.escHtml(...)` i wszystkie bramki pozostały zielone.

### Tests

- **Mobilny test E2E dla kalendarza planu posiłków** (HMAI-417). Siedem dni na cztery pory posiłku to najszerszy układ, jaki aplikacja rysuje, i jedyna strona bez żadnego testu mobilnego. **Samo „brak przewijania poziomego" nie pokrywa reguły responsywnej**: bazowa siatka to `repeat(auto-fit, minmax(170px, 1fr))`, więc na ekranie 393 px mieszczą się *dwie* kolumny — po usunięciu reguły `max-width: 600px` strona nadal się nie przewija, a kalendarz staje się nieczytelny. Doszedł osobny test sprawdzający wprost zwinięcie do jednej kolumny, zweryfikowany przez usunięcie reguły i przebudowanie assetów: test jednokolumnowy pada, test przewijania przechodzi. Atrapa API została wyodrębniona do `tests-e2e/support/` i jest współdzielona przez oba speki — to, co czyni odpowiedź realistyczną (wypełnienie wszystkich dni i pór, 409 przy duplikacie, skalowanie porcjami), to decyzje kontraktowe, a druga kopia jest sposobem, w jaki jedna z nich po cichu zaczyna opisywać API, którego serwer nie ma.

### Documentation

- **Przebudowa README** (HMAI-419). 869 → 291 linii; „Quick start" jest pierwszą sekcją merytoryczną zamiast zaczynać się w linii 152. Materiał referencyjny został **przeniesiony, nie usunięty** — nowy katalog `docs/` z siedmioma plikami (konfiguracja, development, testy i CI, operacje, wyszukiwarka, API, PWA). Przejście udokumentowanej ścieżki od zera wychwyciło **dwie realne dziury**: `make setup` nie uruchamia `npm install` (a `node_modules` jest w `.gitignore`, więc `make assets-prod` na świeżym klonie nie ma czym się uruchomić, a bez builda Twig rzuca 500 na każdej stronie) i nic nie provisionowało indeksu wyszukiwarki — a ta awaria jest cicha, bo nieprovisionowany silnik degraduje się do fallbacku MySQL, który jest równie pusty aż do pierwszego reindeksu. Usunięta została też sprzeczność wewnętrzna: wstęp deklarował „the UI is public", podczas gdy sekcja API poprawnie opisywała HTTP Basic.
- **Przebudowa CLAUDE.md** (HMAI-420). 45 353 → 7 565 słów, czyli sześciokrotne zmniejszenie przy wymaganym czterokrotnym. Plik jest wczytywany do kontekstu na starcie każdej sesji, więc jego objętość to koszt płacony przy każdym zadaniu — a urósł, bo każde zamknięte zadanie było traktowane jako coś do dopisania. Objętość nie była jednak jedynym kosztem: plik pisany jak historia, a czytany jak opis stanu bieżącego, zaczyna wprowadzać w błąd, bo zdanie prawdziwe w chwili zapisu cichnie i zostaje jako reguła, a zdania historycznego **nie da się** skonfrontować z kodem. Zachowane zostały reguły z uzasadnieniami i pułapki, które już kogoś kosztowały dzień; znikła chronologia, klucze zadań i numery wydań jako nośniki informacji. Nowa sekcja „Maintaining this file" zapisuje regułę zapobiegającą ponownemu rozrostowi.

### Coverage

Bez zmiany progu (`COVERAGE_MIN` = 90). Wszystkie pięć zadań CI zielone na każdym z jedenastu PR-ów.

### Migration

1. **Migracja bazy — wymagana.** `make migrate` / `make migrate-test` dokłada indeksy pod odczyty zakresowe. Operacja nieniszcząca.
2. **Odtworzenie kontenera RabbitMQ — jednorazowa utrata DLQ.** Wprowadzenie nazwanego wolumenu startuje od **pustego brokera raz**: cokolwiek stało w kolejce błędów w chwili aktualizacji, przepada. Sprawdź głębokość przed aktualizacją, jeśli to ma znaczenie — `docker compose exec php bin/console messenger:stats`.
3. **Restart workerów — wymagany.** Heartbeat startuje dopiero, gdy workery uruchomią ten kod, więc bez restartu `/api/health` raportuje `worker: degraded`: `docker compose restart messenger_worker scheduler_worker`.
4. **Weryfikacja stanu.** `make doctor` — obraz, klucze, wiek i rozmiar najnowszej kopii, głębokość DLQ.

Bez nowych zmiennych środowiskowych. Bez nowych sekretów. Bez przebudowy obrazu.

### Closed Jira

Epik **HMAI-409** — „Niezawodność, spójność odczytów i dokumentacja finalna" (fixVersion 1.34.0).

Zadania podrzędne: HMAI-410, HMAI-411, HMAI-412, HMAI-413, HMAI-414, HMAI-415, HMAI-416, HMAI-417, HMAI-418, HMAI-419, HMAI-420. HMAI-421 (aktualizacja Confluence) zamykane po wydaniu, żeby dokumentacja opisywała wersję już wydaną.

## [1.33.0] — 2026-08-03

Piętnasty moduł domenowy: **Przepisy** (epik **HMAI-374** — 10 podzadań HMAI-385…394). Katalog przepisów, tygodniowy kalendarz planowania posiłków i lista zakupów generowana z planu, wraz z eksportem CSV/PDF. Tak jak Budżet — **w całości lokalny, bez integracji zewnętrznej i bez nowego OAuth**.

Motywem przewodnim jest **lista zakupów, która się zgadza**. Wszystko w module jest podporządkowane temu, żeby kartka zabrana do sklepu mówiła prawdę: jednostka miary jest enumem, a nie tekstem, bo „g" i „gram" wpisane w dwóch przepisach rozbiłyby jedną pozycję na dwie; ilości są skalowane liczbą zaplanowanych porcji, a nie przepisanych z przepisu; przepis użyty w planie nie daje się usunąć (409 zamiast kaskady), bo wiszące odwołanie skróciłoby listę o cały posiłek i użytkownik dowiedziałby się o tym dopiero przy półce; a jednostki, których nie kupuje się w ułamkach, zaokrąglają się **w górę**, bo jedyny kierunek błędu, na który lista zakupów nie może sobie pozwolić, to zostawić kucharza bez jajka w połowie przepisu.

### Added

- **Domena (HMAI-385).** Agregat `Recipe` (tytuł, uporządkowane składniki i kroki, porcje, opcjonalny czas przygotowania, tagi), VO `Ingredient` i enum `MeasurementUnit`. Tożsamością składnika jest para (nazwa, jednostka) porównywana bez względu na wielkość liter — dokładnie ta sama, po której grupuje lista zakupów, bo „200 g mąki" i „50 g mąki" to na każdej ludzkiej liście jedna pozycja 250 g. Przepis **nie może istnieć bez choćby jednego składnika** (nie da się z niego nic ugotować ani nic do listy dołożyć), ale kroki mogą być puste — część przepisów naprawdę brzmi „wymieszać wszystko".
- **Persystencja (HMAI-386).** Trzy tabele `recipes`/`recipe_ingredients`/`recipe_steps` (migracja `Version20260802000001`) na **czystym DBAL, świadomie bez mapowania ORM** — pierwszy taki agregat w projekcie. Doctrine nie potrafi zmapować kolekcji embeddabli, a jedyne wyjście w stronę ORM wymagałoby nadania `Ingredient` sztucznego identyfikatora, czyli tożsamości obiektowi, który z definicji jej nie ma. Rozstrzyga jednak drugi argument: **Doctrine hydratuje encje z pominięciem konstruktora**, więc `Recipe` byłby jedynym agregatem w systemie, którego niezmienniki nigdy by się nie wykonały. Odczyt przez prawdziwy konstruktor oznacza, że wiersz, którego agregat nigdy by nie zapisał, jest **odrzucany, a nie hydratowany**.
- **Zapisy i odczyty przepisów (HMAI-387, HMAI-388).** Komendy utwórz/podmień/usuń na `command.bus` oraz `GetRecipes` (filtry: tag i fraza) i `GetRecipeDetail` po stronie DBAL. Aktualizacja jest **pełną podmianą** — lista składników to zbiór, który użytkownik przepisuje od nowa, więc pole pominięte byłoby nie do odróżnienia od świadomie wyczyszczonego — i jest **atomowa**: cała nowa treść jest walidowana zanim jakiekolwiek pole zostanie przypisane, żeby odrzucona edycja nie zostawiła przepisu przepołowionego w pamięci wywołującego. Filtr tagu to `JSON_CONTAINS`, nie `LIKE`, bo „otagowany X" znaczy dokładne członkostwo, a wildcardy wpisane przez użytkownika są escapowane — inaczej szukanie `100%` zwracałoby każdy tytuł zawierający `100`.
- **Plan posiłków (HMAI-389, HMAI-390).** Agregat `PlannedMeal` i enum `MealSlot`, tabela `meal_plan` (migracja `Version20260803000001`, tym razem **mapowana ORM** — płaski wiersz skalarów nie ma żadnego z powodów, dla których `Recipe` poszedł w DBAL). Slot trzyma **listę**, nie jeden przepis: polski obiad to rutynowo zupa plus drugie danie, więc reguła „bez duplikatów" została przemierzona na to, co faktycznie jest duplikatem — ten sam przepis dwa razy w tym samym slocie tego samego dnia (409, wymuszone również indeksem unikalnym). Odczyt kalendarza jest **wypełniony w obu wymiarach**: każdy dzień okna i wszystkie cztery sloty w kolejności doby, także puste, żeby klient rysował siatkę wprost z odpowiedzi, nie znając ani słownika slotów, ani jego kolejności.
- **Lista zakupów (HMAI-391).** `GetShoppingList` składa okno planu jednym SUM/GROUP BY: `ilość × porcje_zaplanowane / porcje_przepisu`, grupowane po (nazwa, jednostka). Dwie różne jednostki tego samego składnika zostają dwiema pozycjami — zgadywanie, czy 2 szklanki mąki to 250 g, wpisałoby na listę zakupów liczbę, której nikt nie podał. Ilości wracają **niezaokrąglone**, bo precyzja należy się per jednostka i zapiekanie jednej w API byłoby błędne dla każdej innej.
- **REST API (HMAI-392).** `GET/POST/PUT/DELETE /api/recipes` oraz `GET/POST/PATCH/DELETE /api/meal-plan` i `GET /meal-plan/shopping-list`, wersjonowane `/api/v1` z aliasem `/api` (ADR-008). **PUT dla przepisu, PATCH dla przesunięcia posiłku** — różne czasowniki dla prawdziwej różnicy: podmiana przepisu jest pełna, a przesunięcie zmienia dzień i slot, zostawiając przepis i porcje w spokoju. Okno `from`/`to` jest wymagane na obu odczytach planu; arytmetyka granicy tygodnia nie ma nic do roboty w warstwie HTTP, a wymyślony domyślny zakres groziłby pewną odpowiedzią o tygodniu, o który nikt nie pytał.
- **Panele `/recipes` i `/meal-plan` (HMAI-393).** Stimulus + Encore. Katalog z filtrami i formularzem o dynamicznie dodawanych wierszach składników i kroków, kalendarz tygodniowy z nawigacją i przenoszeniem posiłków przez kliknięcie (ponowne kliknięcie anuluje, żeby pomyłka nie wymuszała wybrania przypadkowego celu) oraz lista zakupów dla widocznego tygodnia. Okno kalendarza liczone jest z **części daty, a nie przez dodawanie milisekund** — tydzień ze zmianą czasu nie ma 7 × 24 h i marsz arytmetyczny przesunąłby dzień.
- **Eksport listy zakupów (HMAI-394).** `GET /api/meal-plan/shopping-list/export?from=&to=&format=csv|pdf`. Lista jest pobierana przez `query.bus`, a nie odpytywana ponownie w eksporterze — kartka zabrana do sklepu ma być dokładnie tym, co pokazał ekran, i to jest cały sens tego pliku. Ilości są tu **zaokrąglane**, bo dokument czytany przez człowieka nie może wydrukować `0.30000000000000004`; CSV niesie kanoniczny identyfikator jednostki (arkusz po nim sortuje i grupuje, a „szt." to kiepski klucz), PDF — polską nazwę. Nazwa pliku zawiera zakres dat, więc trzy wyeksportowane tygodnie to trzy rozróżnialne pliki.
...
1. **Migracja bazy — wymagana.** `make migrate` (dev) / `make migrate-test` (test). Dochodzą cztery tabele: `recipes`, `recipe_ingredients`, `recipe_steps` i `meal_plan`.
2. Brak nowych zmiennych środowiskowych, brak nowych sekretów, brak przebudowy obrazu.

### Closed Jira

HMAI-374 (epik), HMAI-385, HMAI-386, HMAI-387, HMAI-388, HMAI-389, HMAI-390, HMAI-391, HMAI-392, HMAI-393, HMAI-394.
...
Przegląd epiku znalazł **dwa prawdziwe defekty**. Oba należą do klasy „każda funkcja z osobna jest poprawna, ich styk już nie" i oba są wariantem motywu z 1.31.0 — porażki, która udaje sukces.

- **Typ transakcji nie był porównywany z typem kategorii, pod którą ją księgowano.** Reguła była zapisana w trzech miejscach (docblock `Category` mówi wprost, że kategoria nie może mieszać obu przepływów, i właśnie dlatego jej `type` jest niezmienny), ale nie była nigdzie sprawdzana — kategoria była wczytywana wyłącznie po to, żeby potwierdzić, że istnieje. Obie połowy modułu czytały wtedy ten sam wiersz inaczej: lista i eksport filtrują po typie **transakcji**, a raport miesięczny przypisuje całą sumę kategorii do typu **kategorii** (grupowanie po kategorii to jedyny sposób, żeby policzyć wydatki-kontra-limit jednym zapytaniem). Przychód zaksięgowany pod kategorią wydatkową wracał więc jako **201 Created**, na liście figurował jako przychód, w raporcie liczył się jako wydatek, obciążał limit tej kategorii i **przestawiał saldo miesiąca o podwójną kwotę** — bez żadnego sygnału. Teraz to 422. Świadomie odrzucenie, a nie ciche poprawienie typu: nadpisanie tego, co przysłał klient, byłoby przyjęciem złych danych i cichą zmianą ich znaczenia, czyli dokładnie tym, co ta poprawka usuwa. Formularz też się zmienił — typ nie jest już wybierany obok kategorii, tylko z niej wynika, bo tłumaczenie użytkownikowi błędu 422 za wybór, który sam interfejs mu zaproponował, to ten sam błąd w ładniejszym opakowaniu.
- **Każda data była parsowana pobłażliwie, więc data nieistniejąca była po cichu „przewijana", a nie odrzucana.** `createFromFormat` zamienia `2026-02-31` na **3 marca**, a `2026-13` na **styczeń 2027**. Literówka w dniu kończyła się więc księgowaniem pieniędzy w następnym miesiącu (luty zaniżony, marzec zawyżony), a `?month=2026-13` zwracał **200 z liczbami stycznia 2027 podpisanymi jako „2026-13"** — zła odpowiedź podana jako dobra, co w księdze jest gorsze od błędu. Oba parsowania są teraz ścisłe (wartość, która nie formatuje się z powrotem dokładnie na to, co przyszło, nigdy tą datą nie była). Parsowanie miesiąca zostało przy okazji wyciągnięte do wspólnego `MonthRange` — było skopiowane do trzech czytelników (lista, raport, eksport), a powielona pobłażliwość to właśnie sposób, w jaki czwarty czytelnik po cichu przywróciłby ten błąd.

### Documented

- **Kontrakt OpenAPI dla całego modułu (dług świadomie odłożony z HMAI-382 na przegląd epiku).** Wszystkie **11** operacji Budgetu ma atrybuty `#[OA\*]`+`#[Model]` pod nowym tagiem `Budget`, z współdzielonymi odpowiedziami błędów przez `$ref` — dzięki temu **każdy udokumentowany moduł `^/api/*` znów jest objęty kontraktem**. Doszedł `BudgetApiDocTest` (statyczny kontrakt, na równi z pozostałymi modułami) oraz trzy przypadki w **bramce zgodności czasu wykonania** (`OpenApiContractTest`), zasilane przez prawdziwe endpointy zapisu, żeby raport obejmował kategorię z limitem, bez limitu i bez żadnej aktywności. **Żadnego rozjazdu nie wykryto** — normalizery od początku zgadzały się ze schematami, a OpenAPI 3.1 poprawnie zakodował pola nullowalne jako unię typów, nie jako flagę `nullable` z 3.0.

### Known limitation

- **Moduł przyjmuje dowolny kod waluty ISO 4217 per transakcja, ale sumuje bez oglądania się na niego.** Raport dodaje kwoty w obrębie kategorii niezależnie od waluty i porównuje wynik z limitem zapisanym w jednej konkretnej walucie — 100 EUR plus 100 PLN raportuje się jako `20000` bez jednostki, a kategoria z limitem 500 PLN i wydatkiem 400 EUR pokazuje 80% wykorzystania. Nic w module nie jest wielowalutowe (brak przeliczników, brak waluty per kategoria, interfejs wysyła wyłącznie `PLN`), więc uczciwe wyjścia są dwa: wymusić jedną walutę albo zbudować prawdziwą obsługę wielu walut. Wybór między nimi jest decyzją produktową, nie przeglądową, i dlatego jest tu zapisany, a nie rozstrzygnięty.

### Coverage

- **+170 testów PHP** (2121 vs 1951 w 1.31.0), **+16 Playwright** (137), **+18 Vitest** (174) i **+36 asercji Newman** (79). PHPStan level 8 clean, deptrac **0 violations / 0 skip_violations** — moduł nie potrzebował ani jednego wyjątku od reguł architektury.
- Domknięta luka pokrycia: `BudgetRequestParserTest` (unit) wyrównuje moduł do `SeriesRequestParserTest`/`MoviesRequestParserTest`. Reguły parsera były wcześniej sprawdzane wyłącznie end-to-end przez HTTP, co nie pozwala pokryć każdego złego kształtu bez osobnego żądania. Przypina m.in. to, że **kwota zmiennoprzecinkowa jest odrzucana, a nie rzutowana** — `49.99` obcięte do 49 groszy to dokładnie ta cicha strata pieniędzy, przed którą chroni zapis w groszach.
