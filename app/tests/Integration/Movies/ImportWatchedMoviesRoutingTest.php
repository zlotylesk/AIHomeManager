...
class ImportWatchedMoviesRoutingTest extends KernelTestCase
{
    ...
    public function testImportCommandIsRoutedToAsyncTransport(): void
    {
        ...
        $filtered = array_filter(
            ...
            static fn ($envelope) => $envelope->getMessage() instanceof ImportWatchedMoviesFromTrakt,
        );

        self::assertCount(1, $filtered, 'ImportWatchedMoviesFromTrakt must be routed to async — the import is rate-limited + I/O bound and must never run inline in a request.');
    }
}
