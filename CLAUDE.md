...
- CI: `.github/workflows/ci.yml` — 4 jobs on every push/PR: `static-analysis` (Rector dry-run + CS Fixer + PHPStan level 8 + Deptrac + Composer audit), `tests` (PHPUnit + coverage threshold gate, HMAI-245), `e2e-playwright` and `e2e-newman` (both `needs: tests`)
