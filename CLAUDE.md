...
**Status:** projekt operacyjny, ostatni tag `1.17.0` (wydanie utrzymaniowe — epik HMAI-227: runtime PHP 8.4→8.5, obrazy infrastruktury podniesione/przypięte do wspieranych linii [MySQL 8.4 LTS, Redis 8, RabbitMQ 4.x po EOL 3.12, Graylog 6.3 + MongoDB 7], build frontu na Encore 7 / Babel 8 / webpack-cli 7 ESM, in-range bumpy Composer/npm [doctrine-migrations-bundle 4, php-cs-fixer, webpack, @playwright/test]; Symfony świadomie wstrzymane na 8.0.\* — HMAI-225 odroczone, regresja `allow_no_handlers` w framework-bundle 8.1.0. Bez zmian w modelu domenowym; 930 PHP / 52 Playwright / 43 Newman bez zmiany liczby testów. Poprzednie: 1.16.0 Books+Music GUI, 1.15.0 Articles GUI, 1.14.0 Tasks GUI). Sześć modułów domenowych w produkcji. Pełna historia → [CHANGELOG.md](CHANGELOG.md).
