...
- **Shared kernel (HMAI-235):** cross-cutting Domain value objects that legitimately belong to more than one bounded context live in `src/Shared/Domain/ValueObject/` (`App\Shared\…`), NOT duplicated per module. Currently: `CoverUrl` (Books + Series). Deptrac has a dedicated `Shared` layer that every `*Domain` may depend on (and it has zero outbound deps itself). Use it ONLY for genuinely shared, stable domain primitives — module-specific VOs stay in the module. The default is still "no cross-module coupling"; the shared kernel is the one sanctioned exception.
...
- **Deptrac**: formalizes the hexagonal boundaries — every module has separate `*Domain` / `*Application` / `*Infrastructure` layers, plus a cross-cutting `Shared` kernel layer (`src/Shared/.*`, HMAI-235). Domain → [`Shared`] (otherwise zero dependencies beyond PHP core), Application → its own Domain + `Shared` + Vendor, Infrastructure → its own Domain + its own Application + `Shared` + Vendor, `Shared` → [] (pure VOs), `Glue` (Controllers/EventListeners/Kernel/Security outside `src/Module/` and `src/Shared/`) → everything. Cross-module coupling forbidden (the shared kernel is the sole sanctioned cross-context reuse). Config: `app/deptrac.yaml` with a merged `skip_violations` — after HMAI-233 only the cross-module crypto/token sharing remains (Music/Tasks/Series Infrastructure → `App\Security\TokenCipher`; YouTube adapter → Tasks `GoogleTokenRepositoryInterface`); the four Domain-port→Application-DTO entries were dropped once the ports moved to `Domain\ReadModel\`. Regeneration: `make deptrac-baseline` → move `skip_violations` from `deptrac-baseline.yaml` into `deptrac.yaml` and delete the separate file (single source of truth)
