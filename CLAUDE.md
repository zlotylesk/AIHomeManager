...
Single-user system automatyzacji codziennych czynności. Stack: PHP 8.5 + Symfony 8 + MySQL 8.4 LTS + Redis 8 + RabbitMQ 4.x. Heksagonalna architektura, CQRS z dwoma busami.
...
**Status:** projekt operacyjny, ostatni tag `1.16.0` (Books + Music — uzupełnienie GUI: Books dostał edycję metadanych, usuwanie, widok szczegółów z historią sesji czytania, eksport CSV/PDF i dodawanie pełnym payloadem bez ISBN na torze Encore+Stimulus; Music dostał widok lokalnej historii odsłuchów z filtrem from/to/source i ręczny zapis sesji na torze Twig + vanilla JS — wszystko czysty front nad gotowym REST, bez zmian w modelu domenowym. Epic review domknął higienę testów: Books — konsolidacja redundantnego `CoverUrlTest` do kanonicznego `Domain/ValueObject/`; Music — `music.mobile.spec.ts` (ostatni moduł GUI bez mobilnego speca) + testy VO `AlbumArtist`/`AlbumTitle`. **Tym wydaniem cały backlog uzupełnienia GUI — 4 epiki Tasks/Articles/Books/Music — jest domknięty**; każdy moduł ma pełny panel i mobilny E2E guard). Sześć modułów domenowych w produkcji. Pełna historia → [CHANGELOG.md](CHANGELOG.md).
...
- YouTubeProgress panel (`/youtube-progress`): `YouTubeProgressController` (`^/api/youtube-progress/*`) — `GET watchlist` + `GET sessions` czytają wprost przez Domain repos (brak query layer); `POST sync` (dispatch `SyncWatchlist`+`RegenerateSessions`, 400 gdy `YOUTUBE_WATCHLIST_PLAYLIST_ID` puste), `POST videos/{id}/start|watched`, `POST sessions/{id}/push-to-youtube` dispatchują command handlery (404/idempotencja w handlerach, unwrap przez `ApiExceptionListener`). Strona Twig route'owana z `FrontendController` jak reszta nav.
- Selektor ocen Series: 10 przycisków (NIE `<input type=number>`)
- Series — własna ocena: `Series` i `Season` mają **własny, opcjonalny `?Rating`** (VO → kolumna `rating_value` na `series` i `series_seasons`, nullable; ten sam wzorzec co `Episode`), niezależny od średniej z odcinków. **Mapowanie `?Rating` przez custom DBAL type `series_rating` (`Infrastructure/.../Doctrine/Type/RatingType.php`, wzorzec `SeriesStatusType`), NIE embeddable:** nullable embeddable hydruje się jako **non-null** obiekt z niezainicjalizowanym `$value` gdy kolumna jest NULL — niegroźne dla write-path (set-only) i read-path (DBAL), ale wywala każdy odczyt zhydratyzowanego VO (import ocen z Trakt). Custom type round-tripuje `null` czysto w obie strony, więc `Series/Season/Episode::rating()` zwraca prawdziwe `null`. Aggregat: `Series::rate()` / `Series::rateSeason()` (deleguje do `Season::rate()`) + czyszczenie `Series::clearRating()` / `Series::clearSeasonRating()` (deleguje do `Season::clearRating()`) — bez Domain Eventu (brak subscribera, YAGNI). Komendy `RateSeries`/`RateSeason` na `command.bus` (pole `?int $rating` — `null` = clear). Endpointy `PATCH /api/series/{id}/rating` i `PATCH /api/series/{seriesId}/seasons/{seasonId}/rating` (body `{rating:1..10}` → 204 ustawia; `{rating:null}` → 204 czyści własną ocenę; 422 spoza zakresu LUB brak klucza `rating` — `parseRating` rozróżnia jawny `null` od braku przez `array_key_exists`; 404 brak). `GET /api/series/{id}` zwraca dla serialu i sezonu rozłączne `rating` (własna) ORAZ `averageRating` (liczona z odcinków). Kontrolki „My rating" w nagłówku serialu i sezonu (`series_controller.js`, re-use `renderRatingSelector`); przy ustawionej ocenie przycisk „✕" czyści (PATCH `{rating:null}`)
- Series — odcinek obejrzany: `Episode` ma flagę `watched` (bool, NOT NULL DEFAULT 0) + nullable `watchedAt` (`datetime_immutable`) — kolumny `watched`/`watched_at` na `series_episodes` (mapping deklaruje ten sam `default: 0`, bez tego `schema:validate` driftuje i raw-insert testy lecą na „no default value"). Aggregat: `Series::setEpisodeWatched()` (deleguje do `Episode::markWatched(?$watchedAt)` / `unmarkWatched()`) — bez Domain Eventu (YAGNI), opcjonalny `?$watchedAt` zostawia furtkę importowi Trakt na realną datę. Komenda `SetEpisodeWatched` na `command.bus`. Endpoint `PATCH /api/series/{seriesId}/seasons/{seasonId}/episodes/{episodeId}/watched` (body `{watched:bool}` → 204, 422 gdy nie-bool, 404 brak serialu/sezonu/odcinka). `GET /api/series/{id}` zwraca per-odcinek `watched`/`watchedAt` oraz liczniki `watchedCount`/`episodeCount` na poziomie sezonu i serialu. UI: kolumna „Watched" (checkbox `.js-episode-watched`) + licznik `x/y watched` w nagłówku sezonu (`series_controller.js`)
- Series — usuwanie: `DELETE /api/series/{id}` (kaskada sezony+odcinki), `DELETE /api/series/{seriesId}/seasons/{seasonId}` (kaskada odcinki), `DELETE /api/series/{seriesId}/seasons/{seasonId}/episodes/{episodeId}` — wszystkie 204, 404 gdy brak. Komendy `DeleteSeries`/`DeleteSeason`/`DeleteEpisode` na `command.bus`. Agregat: `Series::removeSeason()` / `Series::removeEpisode()` (deleguje do `Season::removeEpisode()`) zwracają usunięty byt i rzucają `DomainException` (→404) przy braku. **Kaskada jawnie w repo** (`SeriesRepositoryInterface::delete/deleteSeason/deleteEpisode` → `EntityManager::remove` + flush) — agregat NIE ma asocjacji ORM (encje persystowane ręcznie przez string-FK), więc `orphanRemoval`/`cascade` nie zadziała. Handlery inwalidują Redis `series:avg:{id}` / `season:avg:{id}` po usunięciu. UI: przyciski kosza (🗑) w nagłówku serialu, sezonu i wierszu odcinka z `confirm()` (`series_controller.js`)
- Series — edycja: `PATCH /api/series/{id}` (body `{title}`), `PATCH /api/series/{seriesId}/seasons/{seasonId}` (body `{number}`), `PATCH /api/series/{seriesId}/seasons/{seasonId}/episodes/{episodeId}` (body `{title}`) — wszystkie 204, 422 (pusty/`>255` tytuł lub numer `<1`/nie-int), 404 brak. **Renumber sezonu na numer już użyty w serialu → 409 Conflict** (dedykowany `Domain\Exception\SeasonNumberAlreadyTaken extends DomainException`; kontroler sprawdza go PRZED generycznym `DomainException`, bo go rozszerza). Komendy `RenameSeries`/`RenumberSeason`/`RenameEpisode` na `command.bus`; agregat `Series::rename()`/`renumberSeason()`/`renameEpisode()` (`Season::renumber()`, `Episode::rename()`) — pola `title`/`number` przestały być `readonly`, persystencja przez zwykły `save()` (Doctrine UoW change-tracking encji managed, brak nowej metody repo). Walidacja tytułu wspólna: `SeriesController::parseTitle()` (re-use w create/addEpisode/rename). UI: inline-edit (klik tytuł/numer → input, Enter/blur zapis, Esc anuluj) — `buildInlineEditable()` w `series_controller.js`
- Series — metadane katalogowe: `Series` zyskuje 4 **opcjonalne** pola: `?string $coverUrl` (kolumna `cover_url`, walidowane VO `Series\Domain\ValueObject\CoverUrl` — kopia wzorca Books, http/https + `FILTER_VALIDATE_URL`; cross-module reuse zakazany przez deptrac więc każdy moduł ma własną kopię), `?int $year`, `?SeriesStatus $status` (enum `Domain\Enum\SeriesStatus` = `ongoing`/`ended`, kolumna `status` przez custom DBAL type `series_status` w `Infrastructure/.../Doctrine/Type/SeriesStatusType.php` — wzorzec `BookStatusType`, nullable round-trip; zarejestrowany w `doctrine.yaml`), `?string $description` (kolumna `description` LONGTEXT = Doctrine `text`). Migracja `Version20260613000001` (4 nullable kolumny). Agregat: `Series::updateMetadata(?coverUrl, ?year, ?status, ?description)` = **full replace** (każde pole nadpisywane, `null` czyści) — bez Domain Eventu. `POST /api/series` i `PATCH /api/series/{id}` przyjmują `{coverUrl, year, status, description}` (camelCase); walidacja w `SeriesController::parseMetadata()` (coverUrl przez VO → 422; year `int` w `[1900, rok+5]`; status przez `SeriesStatus::tryFrom`; description ≤2000 znaków). **PATCH = partial-safe**: title nadal przez `RenameSeries`, metadane dispatchują `UpdateSeriesMetadata` **tylko gdy ≥1 klucz metadanych obecny** (`array_key_exists`) — bare `{title}` (inline edit) NIE zeruje metadanych; pełny formularz edycji wysyła wszystkie 4. Komenda `UpdateSeriesMetadata` na `command.bus` (404 gdy brak serialu). `GET /api/series` + `/{id}` zwracają `coverUrl`/`year`/`status`/`description`. UI: pola w modalu „New Series" + formularz „✎ Edit details" w detalu (`buildEditDetailsForm`/`readMetadataInputs`), miniatura postera (2:3) na karcie listy + poster/rok/status/opis w nagłówku detalu (`series_controller.js`). Regresja: `Unit/.../ValueObject/CoverUrlTest`, `SeriesAggregateTest::testUpdateMetadata*`, `SeriesApiTest` (create+GET metadanych, PATCH update/clear/partial-safe, 422 dla złego coverUrl/year/status)
- Series — realny numer odcinka: `Episode` ma `int $number` (kolumna `number` na `series_episodes`, NOT NULL, bez defaultu) — **unikatowy w obrębie sezonu**, walidowany w `Season::addEpisode()` (rzuca `InvalidArgumentException` → 422 przy duplikacie; pozwala re-add tego samego id = replacement). `POST /api/series/{s}/seasons/{se}/episodes` wymaga teraz `{number:int≥1, title, rating?}` (422 dla nie-int/`<1`/duplikatu). `GET` zwraca `number` per odcinek; odczyt sortuje `ORDER BY e.number` (query handlery), UI renderuje `ep.number` (nie indeks pętli) i sortuje po numerze. Migracja `Version20260612000004` backfilluje istniejące odcinki `ROW_NUMBER() OVER (PARTITION BY season_id ORDER BY id)` (= dotychczasowa kolejność w UI), potem `MODIFY ... NOT NULL`. **Raw-insert testy do `series_episodes` muszą podać `number`** (NOT NULL bez defaultu — `EpisodeRatedHandlerTest`). Formularz add-episode ma pole numeru (default = max+1)
- Series — lista: wyszukiwarka + sortowanie + własna ocena na karcie (**frontend-only** — `GET /api/series` już zwraca komplet): toolbar `#series-toolbar` (input `#series-search` + select `#series-sort`) w `series/index.html.twig`, ukryty gdy zero seriali. `series_controller.js` trzyma pełną listę w `this.allSeries`, `applyListView()` filtruje po tytule (case-insensitive, na żywo) i sortuje przez `sortSeries()` (klucze: `title` A–Z / `rating-desc` średnia ↓ / `own-desc` własna ↓ / `created-desc` najnowsze — oceny `null` na końcu). Pusta lista vs brak dopasowań to dwa różne stany („No series yet" vs „No series match your search"). Karta pokazuje rozłączne `cardRating('My', rating)` + `cardRating('Avg', averageRating)`. E2E: `series.desktop.spec.ts` (filtr zawęża, `created-desc` zmienia kolejność)
- Series — kolorowanie kart/sezonów wg ocen (**frontend-only** — `serializeDTO` współdzielony przez listę i detal zwraca per-serial/per-sezon `rating`/`averageRating`/`watchedCount`/`episodeCount`): subtelne tło kart na liście oraz nagłówków serialu/sezonu w detalu. Helpery w `series_controller.js`: `ratingHighlight(entity)` zwraca `'incomplete'|'mismatch'|null` (**`incomplete` = `episodeCount>0 && watchedCount<episodeCount`** ma **pierwszeństwo** nad `mismatch`, bo średnia z częściowo obejrzanego serialu jest niemiarodajna; `mismatch` = `Math.round(averageRating) !== rating`, a także „jest średnia, brak własnej oceny"), `ratingFlag()` → `{cls,title}` dla nagłówków serialu/sezonu, `cardRatingFlag(s)` → karta: bursztyn gdy serial niekompletny, inaczej czerwony gdy serial **LUB dowolny sezon** ma `mismatch`. Klasy CSS `.is-rating-incomplete` (bursztyn `#fff8e1`) / `.is-rating-mismatch` (czerwony `#fdecea`) w `app.css` (compound z `.series-card`/`.season-header`/`.series-detail-header` dla specyficzności) + `title` tooltip (a11y — kolor nie jest jedynym nośnikiem). E2E: `series.desktop.spec.ts` (stub listy → mismatch=red / incomplete=amber z pierwszeństwem / aligned=neutral)
- Series — import z Trakt (GUI): `POST /api/series/import/trakt` dispatcha `ImportWatchedShowsFromTrakt` (async) → **202** `{status:"import_started"}` natychmiast (import jest rate-limited + I/O bound, NIGDY nie blokuje requestu — wzorzec jak `RefreshDiscogsCollection`). **Brak tokenu Trakt → 409** `{error, authUrl:"/auth/trakt"}` PRZED dispatchem (`SeriesController::isTraktConnected()` czyta `TraktTokenRepositoryInterface::get()` — sam token, bez network I/O; refresh-on-expiry robi worker). UI: przycisk „Import from Trakt" w nagłówku Series (`#btn-import-trakt`, `series_controller.js`) → `API.importFromTrakt()`; 202 pokazuje `#info-banner` („import started…", auto-hide), 409 ustawia `#error-banner` z linkiem `/auth/trakt` (no auto-hide). Banner `#info-banner` w `base.html.twig` (zielony, `.info-banner` w `app.css`). Regresja: `ImportFromTraktApiTest` (409 not-connected bez dispatchu / 202 + dispatch na async transport), E2E `series.desktop.spec.ts` (202 stub → info banner / 409 stub → connect link)
...
| Plik | Rola |
|---|---|
| `app/webpack.config.js` | Encore 7 config (**ESM**: `import Encore`, top-level `await Encore.getWebpackConfig()`, `"type":"module"` w `package.json`) — entry `app`, splitEntryChunks, Stimulus bridge, versioning prod-only, `configureBabel` z `polyfill-corejs3` |
| `app/assets/app.js` | Główny entry — importuje `bootstrap.js` (Stimulus) + `styles/app.css` |
| `app/assets/bootstrap.js` | `startStimulusApp` auto-discovery z `controllers/` przez **`import.meta.webpackContext`** (ESM-native; NIE `require.context` — patrz niżej) |
...
**Encore 7 + Babel 8 + webpack-cli 7 (ESM, HMAI-226):** `webpack.config.js` jest **ESM** (`import Encore`, top-level `await Encore.getWebpackConfig()`, `"type":"module"` w `package.json`). `@symfony/webpack-encore@7.1` deklaruje `peer webpack-cli@^6 || ^7` — `webpack-cli` przypięty do 7.x. Babel 8 (`@babel/core`/`@babel/preset-env` 8.x) usunął opcje `useBuiltIns`/`corejs` z `preset-env`; core-js polyfille wstrzykuje teraz `babel-plugin-polyfill-corejs3` przez `configureBabel` (push `['polyfill-corejs3',{method:'usage-global',version:'3.38'}]`). Targety polyfilli idą przez `"browserslist": ["defaults"]` w `package.json` (Encore ustawia preset-env `targets:{}` → fallback na browserslist; plugin polyfilla czyta ten sam browserslist) — `defaults` (baseline 2026) natywnie wspiera `at`/`replaceAll`/`fromEntries`/`allSettled`, więc nasz nowoczesny kod dostaje **0 polyfilli**. **Stimulus bootstrap pod ESM (GOTCHA):** `assets/bootstrap.js` ładuje kontrolery przez `import.meta.webpackContext` (**NIE** `require.context`) — `"type":"module"` każe webpackowi parsować pliki `.js` assetów jako `javascript/esm`, gdzie CommonJS-owy `require.context` zostaje surową referencją `require` → runtime `ReferenceError: require is not defined` przy starcie, martwy Stimulus (zero kontrolerów się nie montuje). Build **kompiluje się mimo to** (free var ≠ błąd kompilacji), więc bug łapie dopiero E2E w przeglądarce (sam zielony `npm run build` nie wystarcza — trzeba realnego boota strony). `import.meta.webpackContext` to webpack-5-owy ESM-natywny odpowiednik, który parser transformuje w module ESM. Trzy reguły `ignore` w `.github/dependabot.yml` (`webpack-cli` + `@babel/core` + `@babel/preset-env`) **usunięte** — Encore 7 obejmuje te majory peer-range'em, więc Dependabot znów je odświeża. **Caveat na przyszłość:** kolejny major peera Encore'a poza jego range (np. `webpack-cli` 8 spoza `^6||^7`) znów wywali `npm ci` na `ERESOLVE` — nie mergować majora peera zanim Encore nie poszerzy peer-range (precedens: feralny auto-merge webpack-cli 7 / Babel 8 na Encore 6 → hotfixy PR #212/#231).

**Held dependency bump (HMAI-224, PHP 8.5 maintenance):** po domknięciu Encore 7 (HMAI-226) **Symfony jest jedynym świadomie wstrzymanym bumpem** na docelowym stacku PHP 8.5 — pozostaje jako jedyny `outdated`, czeka na osobny ticket HMAI-225:
...
(`newman 7` nadal nie istnieje — latest 6.2.2 — root zostaje na newman 6.x; patrz sekcja niżej.)
...
| Redis 8 | `redis:6379` | Klucze `series:avg:{id}`, `season:avg:{id}` (TTL 3600) ustawiane bezpośrednio przez `\Redis` w `EpisodeRatedHandler` (nie przez Symfony cache pool — handler iniektuje `\Redis`). Pool `cache.rate_limiter` używany przez RateLimiter |
| RabbitMQ 4.x | `rabbitmq:5672` (AMQP), `:15672` UI (guest/guest) | Obraz przypięty do `rabbitmq:4-management-alpine` (3.12 było EOL; major-pin `:4` trzyma wspieraną linię 4.x — Khepri metadata backend, quorum queues default). Transport `async`, exchange `series_events` (topic) + **classic queues** (NIE mirrored — usunięte w 4.0; my ich nie używamy, więc bump = niskie ryzyko BC), retry 3× (1s→2s→4s, max 30s), DLQ `failed`. Metadata efemeryczna (brak data volume) — restart = świeży broker, Messenger auto-deklaruje exchange/kolejki przy połączeniu |
...
Async messages routowane do `async` transportu: `Series\Domain\Event\EpisodeRated`, `Series\Application\Command\ImportWatchedShowsFromTrakt` (import obejrzanych seriali z Trakt offloaded z requestu, pinned `ImportWatchedShowsRoutingTest`), `Series\Application\Command\ImportRatingsFromTrakt` (import ocen z Trakt, chainowany po imporcie obejrzanych, pinned `ImportRatingsRoutingTest`), `Music\Application\Command\RefreshDiscogsCollection` (fetch kolekcji Discogs offloaded z requestu, endpoint `/api/music/collection` zwraca cache + dispatcha refresh przy miss), `Music\Application\Command\PollLastFmRecentTracks` (scheduler poll co 30 min, handler dispatchuje `LogListeningSession` per track na sync command.bus). `Books\Domain\Event\BookCompleted` świadomie sync (in-memory) — brak handlera, brak I/O side-effects (ADR-006). Pinned przez `BookCompletedRoutingTest`.
...
| Logi | `make logs` (all) / `make logs-{php,nginx,mysql,redis,rabbitmq,worker,scheduler,node}` (per-service) |
| Routing | `make routes` |
...
- Async (Messenger): propagacja ID do workera świadomie poza scope — wymaga osobnego Stampa + middleware.
...
- **Disk probe** ma 3 stany, pozostałe nadal binarne up/down:
    - `< 80% used` → `up`
...
- **Dependabot**: `.github/dependabot.yml` — 4 ekosystemy: composer (`/app`, weekly Mon, grupy `symfony/*`/`doctrine/*`/dev), npm (`/app` + `/`, weekly), github-actions (`/`, monthly). PR-y od `dependabot[bot]` przechodzą ten sam CI gate co user commits — review + merge gdy zielone. Dependabot pokrywa freshness, `composer audit`/`npm audit` pokrywa severity-gated regression — komplementarne, nie zastępują się
...
- Limitery (`app/config/packages/rate_limiter.yaml`): `api_per_ip` (sliding_window, 60/min), `discogs_api` (token_bucket, 60/min), `lastfm_api` (token_bucket, 5/s), `national_library_api` (token_bucket, 60/min), `youtube_api` (token_bucket, 60/min — soft HTTP fallback pod unit-based YT quota 10000/dzień), `trakt_api` (token_bucket, 1000/5min — klient importu Trakt)
- `TraktApiClient` (`Series/Infrastructure/External`): `fetchWatchedShows()` → `GET https://api.trakt.tv/sync/watched/shows?extended=full` z nagłówkami `trakt-api-version: 2` + `trakt-api-key: {TRAKT_CLIENT_ID}` + `Authorization: Bearer {access_token}` (token czytany z `TraktTokenRepositoryInterface::get()`, NIE z ENV). Zwraca `list<array{traktId:int, title:string, year:?int, seasons:list<...episodes...>}>` (bez cache — import czyta świeży stan; tytuły odcinków Trakt tu nie podaje, tylko numery + `last_watched_at`). `RuntimeException` gdy brak tokenu („not connected") / pusty client ID / transport error. Wynik konsumuje import biblioteki Trakt do modułu Series. `fetchRatings()` → trzy GET `/sync/ratings/{shows,seasons,episodes}` (te same nagłówki), zwraca `array{shows,seasons,episodes}` z ocenami 1–10 (port `RatingsProviderInterface`); współdzielony prywatny `get()` robi guard tokenu/klienta + obsługę błędów dla obu metod
- Import Trakt → Series: komenda `ImportWatchedShowsFromTrakt` (`Series/Application/Command`, **bez payloadu** — single-user) + handler `#[AsMessageHandler(bus:'command.bus')]`, routing `async` (RabbitMQ; `in-memory` w teście, pinned `ImportWatchedShowsRoutingTest`). Mapuje obejrzane seriale z Trakt na agregat `Series` (+ sezony + odcinki z flagą `watched` i realnym `watchedAt` z `last_watched_at`), **idempotentnie**: dedup serialu po `trakt_id`, sezonu po `number` w serialu, odcinka po `number` w sezonie — ponowny import nie tworzy duplikatów ani nie nadpisuje już-obejrzanych odcinków (`save()` tylko gdy coś się zmieniło). Kryterium importu serialu = ≥1 obejrzany odcinek; pustych seriali/sezonów nie materializuje. Tytuły odcinków Trakt tu nie podaje → placeholder `Episode {n}` (user może zmienić). Brak tokenu Trakt → `RuntimeException` z klienta propaguje (worker retry/DLQ; obsługa UX w osobnej warstwie). Regresja: `ImportWatchedShowsFromTraktHandlerTest` (świeży import / idempotencja / match-by-`trakt_id` + flip watched / skip pustego serialu / propagacja braku tokenu)
- Import ocen z Trakt: komenda `ImportRatingsFromTrakt` (`Series/Application/Command`, **bez payloadu**) + handler, routing `async` (pinned `ImportRatingsRoutingTest`), **chainowana na końcu** `ImportWatchedShowsFromTraktHandler` (oceny lecą po obejrzanych, gdy seriale/sezony/odcinki już istnieją — jeden przycisk „Import from Trakt" robi oba). Mapuje oceny Trakt 1–10 na **własne** oceny agregatu (`Series::rate()`/`rateSeason()`/`rateEpisode()`), **skip-if-missing** (ocena dla nie-zaimportowanego serialu/sezonu/odcinka pomijana — bez materializacji niedoobejrzanych bytów) + **idempotentnie** (`save()` tylko gdy ocena różna od zapisanej; grupowanie po `trakt_id` → 1 load/save na serial). Oceny odcinków świadomie **NIE** dispatchują `EpisodeRated` (bulk = tysiące eventów Redis-recompute; średnie liczone live w `SeriesController::serializeDTO`). Regresja: `ImportRatingsFromTraktHandlerTest` (apply show/season/episode / skip-missing / idempotencja), `TraktApiClientTest::testParsesRatingsIntoStructuredShape`, chain w `ImportWatchedShowsFromTraktHandlerTest::testChainsRatingsImportAfterWatchedShows`
...
- Google OAuth2: `GoogleOAuthTokenRepository` (Tasks) — szyfruje cały `token_json` (access+refresh+expires). Scope claims kumulatywne na refresh tokenie: `calendar.events` (Tasks) + `youtube` (YouTubeProgress, full read/write — wymagane przez `createPlaylist`). Jeden token, dwa moduły — `GoogleClientFactory::create()` requestuje oba scope'y, `setPrompt('consent')` wymusza re-consent. **Po poszerzeniu scope o `youtube` user MUSI raz przejść `/auth/google`** żeby uzyskać token z poszerzonym scope — bez tego YT API call zwróci 403. Regresja: `tests/Integration/Module/YouTubeProgress/GoogleClientYouTubeScopeTest.php`
- Trakt OAuth2: `TraktOAuthTokenRepository` (Series/Infrastructure) — szyfruje cały `token_json`, wzorzec Google. Flow `/auth/trakt` (302→`trakt.tv/oauth/authorize` ze `state` w sesji) + `/auth/trakt/callback` (code→token przez `HttpClientInterface`, zapis zaszyfrowany, redirect `/series`). `TraktTokenProvider::getValidAccessToken()` robi refresh-on-expiry (grant `refresh_token`) — warstwa 2 (TraktApiClient) wstrzykuje provider zamiast repo. Tabela `trakt_oauth_tokens` (DBAL, nie-ORM) wykluczona z `doctrine.dbal.schema_filter` jak `google_oauth_tokens`. ENV: `TRAKT_CLIENT_ID`/`TRAKT_CLIENT_SECRET`/`TRAKT_REDIRECT_URI`. Regresja: `tests/Integration/Auth/TraktAuthControllerTest.php`, `tests/Unit/Module/Series/Infrastructure/TraktTokenProviderTest.php`
...
- `/start-task <KEY>` — Jira → branch → implement → PR → Confluence → transition (workflow z preferencjami: skip STOP checkpoints, no Co-Authored-By)
...
6. Branche: `<KEY>-krotki-opis` od `develop`
