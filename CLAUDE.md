...
**Status:** projekt operacyjny, ostatni tag `1.13.0` (domknięcie epiku **HMAI-178** — Series MVP gap-closure: CRUD + watched + realny numer + metadane + lista z kolorowaniem + import z Trakt obejrzanych i ocen). Sześć modułów domenowych w produkcji. Pełna historia → [CHANGELOG.md](CHANGELOG.md).
...
Async messages routowane do `async` transportu: `Series\Domain\Event\EpisodeRated`, `Series\Application\Command\ImportWatchedShowsFromTrakt` (import obejrzanych seriali z Trakt offloaded z requestu, pinned `ImportWatchedShowsRoutingTest`), `Series\Application\Command\ImportRatingsFromTrakt` (import ocen z Trakt — HMAI-220, chainowany po imporcie obejrzanych, pinned `ImportRatingsRoutingTest`), `Music\Application\Command\RefreshDiscogsCollection` (fetch kolekcji Discogs offloaded z requestu, endpoint `/api/music/collection` zwraca cache + dispatcha refresh przy miss), `Music\Application\Command\PollLastFmRecentTracks` (scheduler poll co 30 min, handler dispatchuje `LogListeningSession` per track na sync command.bus). `Books\Domain\Event\BookCompleted` świadomie sync (in-memory) — brak handlera, brak I/O side-effects (ADR-006). Pinned przez `BookCompletedRoutingTest`.
