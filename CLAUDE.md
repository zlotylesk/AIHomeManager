...
**Status:** projekt operacyjny, ostatni tag `1.15.0` (Articles — uzupełnienie GUI: dodawanie, edycja, usuwanie, widok szczegółów, eksport CSV/PDF, import CSV/Pocket przez upload i filtr read/unread na torze Twig + vanilla JS; jedyna zmiana PHP to addytywny `POST /api/articles/import`; epic review dołożył mobilny E2E overflow guard — Articles był ostatnim modułem GUI bez mobilnego speca). Sześć modułów domenowych w produkcji. Pełna historia → [CHANGELOG.md](CHANGELOG.md).
